	...
	float p_miss)
{
	...
	for(int i = 0; i < beliefs.size(); i++){
		for(int j = 0; j < beliefs[0].size(); j++){
			...
			newRow.push_back(beliefs[i][j]*(hit*p_hit+(1-hit)*p_miss));
		}
	...
	for(int x = 0; x < newGrid.size(); x++){
		for(int y = 0; y < newGrid[0].size(); y++){
			s += newGrid[x][y];
		}
	}

	for(int a = 0; a < newGrid.size(); a++){
		for(int b = 0 ; b < newGrid[0].size(); b++){
			newGrid[a][b] = newGrid[a][b]/s;
		}
	}
	...
	float blurring)
{
	...
	for(int a = 0; a < height; a++){
		for(int b = 0 ; b < width; b++){
			int new_i = (int(dy+ a +width))%(width);
			int new_j = (int(dx+ b +height))%(height);
			newGrid[int(new_j)][int(new_i)] = beliefs[a][b];
		}
	}
